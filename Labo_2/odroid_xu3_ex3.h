...
#define CONFIG_EXTRA_ENV_SETTINGS 					\
	...
	"rootpath=/var/lib/tftpboot/odroidxu3/rootfs\0"			\
	"hostname=odroidxu3\0"						\
	"kernel_img=/odroidxu3/xu3-zImage\0"				\
	"rootfs_img=xu3-rootfs.ext4\0"					\
	"fdts_img=/odroidxu3/exynos5422-odroidxu3.dtb\0"		\
	"usbethaddr=00:10:75:2a:ae:e0\0"				\
	"addttyargs=setenv bootargs ${console} earlyprintk debug\0" 	\
	...
	"addmmcargs=setenv bootargs ${bootargs} "			\
		"root=/dev/mmcblk1p2 rw rootwait rootfstype=ext4\0"	\
	"nfsboot=usb start; run addttyargs addnfsargs addipargs; "	\
		"tftpboot ${fdts_addr}   ${fdts_img}; "			\
		"tftpboot ${kernel_addr} ${kernel_img}; "		\
		"bootz ${kernel_addr} - ${fdts_addr}\0"			\
	"update_kernel=usb start; usb reset; "				\
		"tftpboot ${fdts_addr} ${fdts_img}; "			\
		"mmc write 0 ${fdts_addr} 0x58a0 0x100; "		\
		"tftpboot ${kernel_addr} ${kernel_img}; "		\
		"mmc write 0 ${kernel_addr} 0x18a0 0x4000\0"		\
	"update_rootfs=usb start; usb reset; "				\
		"tftpboot ${rootfs_addr} ${rootfs_img}; "		\
		"mmc write 0 ${rootfs_addr} 0x20000 ${rootfslen}\0"	\
	"mmcboot="							\
		"run addttyargs addmmcargs addipargs; "			\
		"ext4load mmc 0:1 ${fdts_addr} exynos5422-odroidxu3.dtb; " \
		"ext4load mmc 0:1 ${kernel_addr} xu3-zImage; "		\
		"bootz ${kernel_addr} - ${fdts_addr}\0"			\
	"erase_env=mmc erase 0x1880 0x20\0"
